...
class StandardSetUpTestCase(TestCase):
    ...
    def create_trade(self, date, bank, money_amount, stock_amount, stock, buy_or_sell):
        trade = TradeForm(self.depot, {'bank': bank, 'date': date, 'money_amount': money_amount,
...
class FormValidationTestCase(StandardSetUpTestCase):
    def setUp(self):
        ...
        self.stock = Stock.objects.create(name='Test Stock', exchange='TEST', ticker='TST', depot=self.depot)
    ...
    def test_trade_can_not_be_inserted_before_another_trade_which_turns_balance_negative(self):
        self.create_flow('2020-05-05T12:40', 1, self.bank)
