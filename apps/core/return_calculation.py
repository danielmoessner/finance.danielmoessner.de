...
def get_time_weighted_return_df(
    ...
    df.loc[:, "twr"] = (df.loc[:, "value"] - df.loc[:, "flow"]) / df.loc[
        :, "value"
    ].shift(1)
...
def get_internal_rate_of_return_df(
    ...
    # test that the length is equal to the length of the
    # flow df plus the last row of the value df
    if (
        len(df.index) != len(flow_df.index) + 1
        and len(df.index) != len(flow_df.index) + 2
    ):
        return None
...
def _custom_xnpv(rate, df):
    ...
    for i in range(0, df.shape[0]):
        xnpv += (
            df.iloc[i, df.columns.get_loc("flow")]
            / (1 + rate) ** df.iloc[i, df.columns.get_loc("days")]
        )
    # return the
    return xnpv
...
def get_internal_rate_of_return(df: pd.DataFrame) -> Union[pd.DataFrame, None]:
    ...
    internal_rate_of_return = (1 + internal_rate_of_return) ** (
        df.iloc[-1, df.columns.get_loc("days")]
    )
