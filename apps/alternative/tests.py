...
class GeneralTestCase(TestCase):
    def setUp(self):
        ...
        self.alternative = Alternative.objects.create(
            depot=self.depot, name="Test Alternative"
        )

    def create_flow(self, date, flow):
        flow = FlowForm(
            self.depot, {"alternative": self.alternative.pk, "date": date, "flow": flow}
        )
        flow.save()
        return flow
    ...
    def create_value(self, date, value):
        value = ValueForm(
            self.depot,
            {"alternative": self.alternative.pk, "date": date, "value": value},
        )
        value.save()
        return value
